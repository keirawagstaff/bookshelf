...
interface Friend { id: string; name: string | null; image: string | null; }
interface PendingRequest { id: string; sender: Friend; }
...
export default function FriendsPage() {
  ...
  async function respondToRequest(friendshipId: string, action: "accept" | "reject") {
    ...
    if (action === "accept" && request) setFriends((prev) => [...prev, request.sender]);
  }
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <section className="mb-10">
        ...
        <div className="relative">
          ...
          {searching && <div className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 border-2 border-gray-300 border-t-black rounded-full animate-spin" />}
        </div>
            ...
            {searchResults.map((u) => (
              <li key={u.id} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm" style={{ fontFamily: "system-ui, sans-serif" }}>{u.name}</span>
                </div>
      ...
      {pending.length > 0 && (
        <section className="mb-10">
          <h2 className="text-lg font-semibold mb-3">
            Pending requests <span className="text-gray-400 font-normal text-sm">({pending.length})</span>
          </h2>
            ...
            {pending.map((req) => (
              <li key={req.id} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm" style={{ fontFamily: "system-ui, sans-serif" }}>{req.sender.name}</span>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => respondToRequest(req.id, "accept")}
                    ...
                    style={{ fontFamily: "system-ui, sans-serif" }}>Accept</button>
                  <button onClick={() => respondToRequest(req.id, "reject")}
                    ...
                    style={{ fontFamily: "system-ui, sans-serif" }}>Decline</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section>
        <h2 className="text-lg font-semibold mb-3">
          My friends <span className="text-gray-400 font-normal text-sm">({friends.length})</span>
        </h2>
        {friends.length === 0 ? (
            ...
            {friends.map((friend) => (
              <li key={friend.id} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm" style={{ fontFamily: "system-ui, sans-serif" }}>{friend.name}</span>
                </div>
                <Link href={`/shelf/${friend.id}`}
                  className="text-xs border border-gray-300 rounded-full px-3 py-1 hover:bg-black hover:text-white hover:border-black transition-colors"
                  style={{ fontFamily: "system-ui, sans-serif" }}>
                  View shelf
                </Link>
